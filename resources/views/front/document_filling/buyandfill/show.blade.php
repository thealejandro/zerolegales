...
@section('pageTitle',__('test.Document | Preview'))
@section('content')
<style>
    @font-face {
                            ...
                            <div class="edit-cert">
                                ...
                                <div class="cert-bt-edit">
                                    @if($document_filling['download_status'] == '1')
                                        <a href="{{route('user.nosubscription.buy.fill.edit',[$document_id,$id])}}" class="btn btn-comon btn-blue-outline disabled">Editar</a>

                                    @else

                                        <a href="{{route('user.nosubscription.buy.fill.edit',[$document_id,$id])}}" class="btn btn-comon btn-blue-outline">Editar</a>
                                ...
                                <div class="cert-process-card" id="before_purchase">
                                        ...
                                        @if(Auth::user()->user_type != 2)                             
                                            <div class="doc-authentication">
                                                ...
                                                @endif
                                                @if($purchases == null)  
                                                    <div class="qnumber">Q {{$price['price']}}</div>
                                        ...
                                        @if(Auth::user()->user_type != 2)                             
                                            <div class="doc-authentication">
                                                @if($purchases)
                                                    <div class="qnumber">Q 297.00</div>
                                                @endif
                                                @if($purchases == null)  
                                                    <div class="qnumber">Q {{$price['price']}}</div>
                                                    <div class="doc-authentication-title">Documento con autenticación</div>
                                        ...
                                        @if(Auth::user()->user_type != 2)
                                            @if($purchases)
                                            <a href="{{route('user.buy.fill.legalisation.invoice',[$template->id,$id])}}" class="btn btn-comon btn-blue legalization disabled" style="display:none;">Comprar</a>
                                            <a href="{{route('user.document.purchase.after',[$document_id,$id])}}" class="btn btn-comon btn-blue comprar" >Comprar</a>
                                            @endif
                                            @if($purchases == null) 
                                            <a href="{{route('user.fill.buy.purchase.invoice',[$template->id,$id])}}" class="btn btn-comon btn-blue legalization disabled" style="display:none;">Comprar</a>
                                            <a href="{{route('user.fill.buy.document.purchase.invoice',[$template->id,$id])}}" class="btn btn-comon btn-blue comprar" >Comprar</a>
                                            @endif
                                            @else
                                            <a href="{{route('user.buy.fill.legalisation.invoice',[$template->id,$id])}}" class="btn btn-comon btn-blue legalization disabled" style="display:none;">Comprar y Continuar</a>
                                            <a href="{{route('user.document.filling.process',[$document_id,$id])}}" class="btn btn-comon btn-blue comprar">Continuar</a>
                                            @endif
                                        </div>
                                        @endif
                                        @if($template->document_authentication == 'yes')
                                        <div class="form-preview-sub">
                                            ...
                                            @endif
                                            @else
                                            <a href="{{route('user.buy.fill.legalisation.invoice',[$template->id,$id])}}" class="btn btn-comon btn-blue legalization disabled">Comprar y Continuar</a>
                                            <a href="{{route('user.document.filling.process',[$document_id,$id])}}" class="btn btn-comon btn-blue comprar" style="display:none;">Continuar</a>
                                            @endif
                                        </div>
                                        @endif
