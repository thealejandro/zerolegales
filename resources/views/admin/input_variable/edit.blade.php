...
@section('pageTitle',__('test.edit-input-variable'))
@section('content')
    <div class="breadcrumb">
        <h1>{{__('test.edit-input-variable')}}</h1>
        <ul>
            <li>{{__('test.home')}}</li>
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card mb-4">
                <div class="card-body">
                    <div class="card-title mb-3">{{__('test.edit-input-variable')}}</div>
                    {!! Form::model($variable,['route' => ['admin.input.variable.update',$id], 'method' => 'POST','class' => 'kt-form','id'=>'edit_variable_form',
                            'role' => 'form','novalidate']) !!}
                        <div class="row">
                            <div class="col-md-6 form-group mb-3">
                                <label>{{__('test.variable-name')}}<span class="required">&#42;</span></label>
                                <input type="text" name="variable_name" id="variable_name" placeholder="{{__('test.variable-name')}}" class="form-control" value="{{ $variable->variable_name }}" autocomplete="off">
                            ...
                            <div class="col-md-6 form-group mb-3">
                                ...
                                <select name="variable_type" class="form-control" id="variable_type">
                                    ...
                                    <option value="{{$type->id}}"{{ $variable->variable_type == $type->id ? 'selected' : '' }}>{{$type->variable_type}}({{$type->description}})</option>
                                    @endforeach                                         
                                </select>
