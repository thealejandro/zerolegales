...
@section('pageTitle',__('test.All legal documents'))
@section('content')
    <div class="breadcrumb">
        <h1>{{ __('test.Report3') }}</h1>
        <!-- <ul>
            <li>{{ __('test.Report') }}</li>
            <li>{{ __('test.Report3') }}</li>
        ...
        <div class="col-md-12 mb-4">
            <div class="card text-left">
                ...
                <div class="row pb-3">
                    ...
                    <div class="col-md-3">
                        <label>{{ __('test.document-status') }}</label>
                        <select class="browser-default custom-select" id="document_status">
                            <option value="">{{ __('test.all') }}</option>
                ...
                <div class="table-responsive">
                    <table id="report3_table" class="display table table-striped table-bordered" style="width:100%">
                        <thead>
                            <tr>
                                <th>{{ __('test.document-name') }}</th>
                                <th>{{ __('test.document-status') }}</th>
                                <th>{{ __('test.subscription-type') }}</th>
                                <th>{{ __('test.user') }}</th>
                                <th>{{ __('test.document-completed-date') }}</th>
                                <th>{{ __('test.sent-for-authentication') }}</th>
                            </tr>
                        </thead>
                        <tbody>                                      
                        </tbody>
    ...
    <script>
        $(function() {
            ...
            table = $('#report3_table').DataTable({
                ...
                buttons: [
                    ...
                    },
                    {
                        ...
                        customize: function(pdf, button, api) {
                            pdf.styles.tableBodyEven.alignment = 'center';
                            pdf.styles.tableBodyOdd.alignment = 'center'; 
                        },
                        messageTop: function() {
                            let text = '';
                            text += 'Filtrado: ' + response.limit;
                            text += '\n';
                            text += 'Total Filtrado: ' + response.recordsFiltered;
                            text += '\n';
                ...
                "scrollX": true,
                "ajax": {
                    url: "{{ route('admin.reports.report3.datatable') }}",
                ...
                drawCallback: function(settings) {
                    ...
                    $('#count').text('Total de documentos completados y pendientes :'+response.recordsFiltered)
                },
